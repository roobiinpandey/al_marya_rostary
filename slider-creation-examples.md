...
## 🎯 **API Endpoint**
```
POST https://qahwatapp.onrender.com/api/sliders
```
...
curl -X POST \
  ...
  -F "buttonLink=https://qahwatapp.com/menu" \
  -F "backgroundColor=#8B4513" \
  -F "textColor=#FFFFFF" \
  -F "position=center" \
  -F "displayOrder=1" \
  -F "isActive=true" \
  https://qahwatapp.onrender.com/api/sliders
```
...
curl -X POST \
  ...
  -F "buttonLink=https://qahwatapp.com/ramadan-special" \
...
curl -X PUT \
  ...
  -F "displayOrder=1" \
  https://qahwatapp.onrender.com/api/sliders/SLIDER_ID_HERE
```
...
async function createSlider(sliderData, imageFile, mobileImageFile = null) {
    ...
    try {
        const response = await fetch('https://qahwatapp.onrender.com/api/sliders', {
            method: 'POST',
            headers: {
...
const sliderData = {
    ...
    buttonLink: 'https://qahwatapp.com/menu',
...
def create_slider(admin_token, image_path, slider_data, mobile_image_path=None):
    url = 'https://qahwatapp.onrender.com/api/sliders'
...
# Get specific slider
curl -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  https://qahwatapp.onrender.com/api/sliders/SLIDER_ID
