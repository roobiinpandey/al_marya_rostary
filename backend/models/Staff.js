...
staffSchema.index({ employeeId: 1, isDeleted: 1 }); // NEW: Primary lookup
staffSchema.index({ qrBadgeToken: 1 }); // NEW: QR badge lookup
...
staffSchema.methods.addLoginHistory = function(method, deviceInfo, ipAddress, success, failureReason = null) {
...
// ===== NEW: Pre-save Middleware for PIN Hashing =====

// Pre-save middleware: Hash PIN if modified
staffSchema.pre('save', async function(next) {
  // Only hash if PIN is modified and is exactly 4 digits (not already hashed)
  if (this.isModified('pin') && /^\d{4}$/.test(this.pin)) {
    try {
