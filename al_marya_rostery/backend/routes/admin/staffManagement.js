...
/**
 * @route   POST /api/admin/staff/:id/grant-admin-access
 * @desc    Grant admin panel access to staff member for product management
 * @access  Private (Admin only)
 */
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Check if staff already has admin access
      if (staff.hasAdminAccess) {
        return res.status(400).json({
          success: false,
          message: 'Staff member already has admin access. Revoke existing access first to change credentials.'
        });
      }
      ...
      if (existingStaff && existingStaff._id.toString() !== staff._id.toString()) {
        return res.status(400).json({
          success: false,
      ...
      // Grant admin access using the Staff model method
      await staff.setAdminCredentials(adminUsername, adminPassword);
      ...
      res.json({
        ...
        credentials: {
          adminUsername: staff.adminUsername,
          note: 'Password has been set. Staff can now login to product management with these credentials.'
        }
      });
...
/**
 * @route   POST /api/admin/staff/:id/revoke-admin-access
 * @desc    Revoke admin panel access from staff member
 * @access  Private (Admin only)
 */
router.post(
  '/:id/revoke-admin-access',
  protect,
  authorize('admin'),
  async (req, res) => {
    try {
      ...
      if (!staff.hasAdminAccess) {
        return res.status(400).json({
          success: false,
          message: 'Staff member does not have admin access'
        });
      }

      console.log('🔒 Revoking admin access:', {
        staffId: staff._id,
        staffName: staff.name,
        adminUsername: staff.adminUsername
      });

      // Revoke admin access using the Staff model method
      staff.removeAdminAccess();
      await staff.save();
...
// ===== Get Staff with Admin Access =====

/**
 * @route   GET /api/admin/staff/with-admin-access
 * @desc    Get list of all staff members with admin access
 * @access  Private (Admin only)
 */
router.get(
  '/with-admin-access',
  ...
  async (req, res) => {
    try {
      const staffWithAdmin = await Staff.find({
        hasAdminAccess: true,
        isDeleted: false
      }).select('name employeeId role adminUsername hasAdminAccess status');

      res.json({
        success: true,
        count: staffWithAdmin.length,
        staff: staffWithAdmin
      });

    } catch (error) {
      console.error('Get staff with admin access error:', error);
      res.status(500).json({
        success: false,
        message: 'Error fetching staff with admin access',
        error: error.message
      });
    }
  }
