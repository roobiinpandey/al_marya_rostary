  ...
  try {
    ...
    try {
      ...
      if (customer && customer.fcmToken) {
        await pushNotificationService.sendToDevice(customer.fcmToken, {
          title: 'Order Picked Up! 🚗',
          body: `Your order #${order.orderNumber} is on its way! Expected delivery in ${order.deliveryTime || '30'} minutes.`,
          data: {
        ...
        console.log(`✅ Delivery notification sent to customer ${customer.name}`);
      }
    } catch (notificationError) {
      console.error('❌ Failed to send pickup notification:', notificationError.message);
