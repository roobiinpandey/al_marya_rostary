...
const staffManager = {
                            ...
                            <div class="modal-body">
                                <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem;">
                                    ...
                                    <div>
                                        ...
                                        <h4 style="margin-top: 1.5rem; margin-bottom: 1rem; color: #A89A6A;">🔐 Product Management Access</h4>
                                        <div style="display: grid; gap: 0.75rem;">
                                            <div>
                                                <strong>Status:</strong> 
                                                ${staff.hasAdminAccess ? 
                                                    '<span style="color: #28a745; font-weight: bold;">✓ Has Access</span>' : 
                                                    '<span style="color: #6c757d;">No Access</span>'
                                                }
                                            </div>
                                            ${staff.hasAdminAccess ? 
                                                `<div><strong>Admin Username:</strong> ${staff.adminUsername}</div>
                                                <div style="background: #e8f5e9; padding: 10px; border-radius: 4px; margin-top: 5px;">
                                                    <small style="color: #2e7d32;">
                                                        <i class="fas fa-info-circle"></i> 
                                                        This staff member can manage products in the Staff App
                                                    </small>
                                                </div>` : 
                                                `<div style="background: #f5f5f5; padding: 10px; border-radius: 4px; margin-top: 5px;">
                                                    <small style="color: #666;">
                                                        <i class="fas fa-info-circle"></i> 
                                                        Grant access to allow product management from Staff App
                                                    </small>
                                                </div>`
                                            }
                                        </div>
                                ...
                                <div style="margin-top: 2rem; display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                                    ...
                                    ${!staff.hasAdminAccess ? 
                                        `<button class="btn btn-success" onclick="staffManager.showGrantAdminAccessModal('${staff.id}', '${staff.name}')" style="background: #4CAF50;">
                                            <i class="fas fa-user-shield"></i> Grant Product Access
                                        </button>` : 
                                        `<button class="btn btn-danger" onclick="staffManager.showRevokeAdminAccessModal('${staff.id}', '${staff.name}')">
                                            <i class="fas fa-user-slash"></i> Revoke Product Access
                                        </button>`
                                    }
                                </div>
                            </div>
    ...
    /**
     * Show grant admin access modal
     */
    showGrantAdminAccessModal(staffId, staffName) {
        const modalHTML = `
            <div class="modal-overlay" id="grantAdminAccessModal" onclick="staffManager.closeModal(event)">
                <div class="modal-content" style="max-width: 500px;" onclick="event.stopPropagation()">
                    <div class="modal-header">
                        <h3><i class="fas fa-user-shield"></i> Grant Product Management Access</h3>
                        <button class="close-btn" onclick="staffManager.closeModal()">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div style="background: #e3f2fd; padding: 15px; border-radius: 4px; margin-bottom: 20px;">
                            <p style="margin: 0; color: #1976d2;">
                                <i class="fas fa-info-circle"></i>
                                <strong>${staffName}</strong> will be able to add, edit, and delete products from the Staff App.
                            </p>
                        </div>

                        <form id="grantAdminAccessForm" onsubmit="staffManager.submitGrantAdminAccess(event, '${staffId}')">
                            <div class="form-group">
                                ...
                                <input 
                                    ...
                                    name="adminUsername" 
                                    class="form-control" 
                                    placeholder="e.g., ${staffName.toLowerCase().replace(/\s+/g, '_')}_admin"
                            ...
                            <div class="form-group">
                                ...
                                <input 
                                    ...
                                    required>
                                <small style="color: #666; display: block; margin-top: 5px;">
                                    Minimum 6 characters. This is separate from the staff PIN.
                                </small>
                            </div>
                            ...
                            <div style="background: #fff3cd; padding: 12px; border-radius: 4px; margin-top: 15px;">
                                <small style="color: #856404;">
                                    ...
                                    <strong>Important:</strong> Save these credentials securely. The staff member will need them to access product management in the Staff App.
                                </small>
                            </div>
     ...
     */
    async submitGrantAdminAccess(event, staffId) {
        event.preventDefault();
        
        const form = event.target;
        const formData = new FormData(form);
        const data = {
            adminUsername: formData.get('adminUsername').trim(),
            adminPassword: formData.get('adminPassword')
        };

        try {
            const token = localStorage.getItem('adminToken');
            const response = await fetch(`/api/admin/staff/${staffId}/grant-admin-access`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (result.success) {
                this.showSuccess(`✅ Admin access granted successfully! Username: ${result.credentials.adminUsername}`);
                this.closeModal();
                // Refresh staff list
                await this.loadStaffList();
                // Reopen staff details modal to show updated buttons
                setTimeout(() => {
            ...
            this.showError('Failed to grant admin access: ' + error.message);
        }
    },

    /**
     * Show revoke admin access modal
     */
    showRevokeAdminAccessModal(staffId, staffName) {
        const modalHTML = `
            <div class="modal-overlay" id="revokeAdminAccessModal" onclick="staffManager.closeModal(event)">
                <div class="modal-content" style="max-width: 450px;" onclick="event.stopPropagation()">
                    <div class="modal-header">
                        <h3><i class="fas fa-user-slash"></i> Revoke Product Management Access</h3>
                        <button class="close-btn" onclick="staffManager.closeModal()">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div style="background: #ffebee; padding: 15px; border-radius: 4px; margin-bottom: 20px;">
                            <p style="margin: 0; color: #c62828;">
                                <i class="fas fa-exclamation-triangle"></i>
                                <strong>${staffName}</strong> will no longer be able to manage products from the Staff App.
                            </p>
                        </div>

                        <p style="margin-bottom: 20px;">Are you sure you want to revoke product management access?</p>

                        <div style="display: flex; gap: 10px; justify-content: flex-end;">
                            <button type="button" class="btn btn-secondary" onclick="staffManager.closeModal()">
                                Cancel
                            </button>
                            <button type="button" class="btn btn-danger" onclick="staffManager.submitRevokeAdminAccess('${staffId}')">
                                <i class="fas fa-check"></i> Yes, Revoke Access
                            </button>
                        </div>
                    </div>
                </div>
    ...
    /**
     * Submit revoke admin access
     */
    async submitRevokeAdminAccess(staffId) {
        try {
            const token = localStorage.getItem('adminToken');
            const response = await fetch(`/api/admin/staff/${staffId}/revoke-admin-access`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            if (result.success) {
                this.showSuccess('✅ Admin access revoked successfully!');
                this.closeModal();
                // Refresh staff list
                await this.loadStaffList();
                // Reopen staff details modal to show updated buttons
                ...
                throw new Error(result.message || 'Failed to revoke admin access');
            }
        } catch (error) {
            console.error('Error revoking admin access:', error);
            this.showError('Failed to revoke admin access: ' + error.message);
        }
    }
};
