...
class EmailService {
  ...
  async sendGuestOrderConfirmation(order, guestInfo) {
    const email = guestInfo?.email || order.guestInfo?.email;
    const name = guestInfo?.name || order.guestInfo?.name;
    ...
    return await this.sendEmail({
      to: email,
      subject: `Your Order #${order.orderNumber} is Confirmed - Qahwat Al Emarat`,
      html
    });
  }
  ...
  async sendOrderCancellation(order, user) {
    ...
    const refundAmount = order.cancellation?.refundAmount || order.totalAmount;
    const refundStatus = order.cancellation?.refundStatus || 'pending';
    ...
    const html = `
      <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
        ...
        <div style="background: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
          <h3 style="margin-top: 0;">Order #${order.orderNumber}</h3>
          <p><strong>Cancellation Reason:</strong> ${order.cancellation?.reason || 'User requested'}</p>
          <p><strong>Original Total:</strong> AED ${order.totalAmount.toFixed(2)}</p>
          <p><strong>Refund Amount:</strong> AED ${refundAmount.toFixed(2)}</p>
          <p><strong>Refund Status:</strong> ${refundStatus.charAt(0).toUpperCase() + refundStatus.slice(1)}</p>
        </div>

        <p style="color: #666;">
          ${refundStatus === 'processing' 
            ? `Your refund is being processed and should appear in your account within 3-5 business days.`
            : `Your refund has been initiated and is pending processing.`}
        </p>
    ...
    return await this.sendEmail({
      to: email,
      subject: `Order #${order.orderNumber} Cancellation Confirmation - Qahwat Al Emarat`,
      html
    });
  }
