  ...
  try {
    ...
    try {
      ...
      if (!user.firebaseUid || user.firebaseUid !== uid) {
        ...
        // CRITICAL FIX: Save sync to MongoDB immediately
        try {
          await user.save();
          console.log(`✅ Firebase UID synced and saved to database for ${user.email}`);
        } catch (syncError) {
          console.error(`❌ CRITICAL: Failed to sync Firebase UID to database:`, {
          ...
          // This allows the request to proceed, but we know sync failed
          // The sync will be retried on the next authentication
        }
      }
