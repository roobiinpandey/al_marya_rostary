...
extension RTLExtension on BuildContext {
  ...
  Alignment get startAlignment => read<LanguageProvider>().getStartAlignment();

  /// Get end alignment (right for LTR, left for RTL)
  Alignment get endAlignment => read<LanguageProvider>().getEndAlignment();

  /// Get start text alignment (left for LTR, right for RTL)
  TextAlign get startTextAlign => read<LanguageProvider>().getStartTextAlign();

  /// Get end text alignment (right for LTR, left for RTL)
  TextAlign get endTextAlign => read<LanguageProvider>().getEndTextAlign();
  ...
  String formatDate(DateTime date) => read<LanguageProvider>().formatDate(date);
...
class I18nUtils {
  ...
  static String getMonthName(BuildContext context, int month) {
    ...
    switch (month) {
      case 1:
        return isArabic ? 'يناير' : 'January';
      case 2:
        return isArabic ? 'فبراير' : 'February';
      case 3:
        return isArabic ? 'مارس' : 'March';
      case 4:
        return isArabic ? 'أبريل' : 'April';
      case 5:
        return isArabic ? 'مايو' : 'May';
      case 6:
        return isArabic ? 'يونيو' : 'June';
      case 7:
        return isArabic ? 'يوليو' : 'July';
      case 8:
        return isArabic ? 'أغسطس' : 'August';
      case 9:
        return isArabic ? 'سبتمبر' : 'September';
      case 10:
        return isArabic ? 'أكتوبر' : 'October';
      case 11:
        return isArabic ? 'نوفمبر' : 'November';
      case 12:
        return isArabic ? 'ديسمبر' : 'December';
      default:
        return '';
    }
  }
